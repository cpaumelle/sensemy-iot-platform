...
export const deviceService = {
  ...
  async getDevices(params = {}) {
    try {
      ...
      const response = await apiClient.get('/v1/devices', { params });
  ...
  async getDevice(deveui) {
    try {
      ...
      const response = await apiClient.get(`/v1/devices/${deveui}`);
  ...
  async updateDevice(deveui, data) {
    try {
      ...
      const response = await apiClient.put(`/v1/devices/${deveui}`, data);
  ...
  async createDevice(data) {
    try {
      ...
      const response = await apiClient.post('/v1/devices', data);
  ...
  // PATCH /v1/devices/{deveui}/archive
  async archiveDevice(deveui, confirm = true) {
    try {
      console.log(`🗑️ Archiving device: ${deveui}`);
      const response = await apiClient.patch(`/v1/devices/${deveui}/archive?confirm=${confirm}`);
  ...
  // GET /v1/devices/full-metadata
  async getDeviceMetadata() {
    try {
      console.log('🔍 Fetching device metadata');
      const response = await apiClient.get('/v1/devices/full-metadata');
